...
class _AuthField extends StatelessWidget {
  ...
  const _AuthField({required this.ctrl, required this.label, required this.hint, this.obscure = false, this.keyboard = TextInputType.text});
  @override
...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (mounted) {
        final sb = await _getCompanyId();
        if (mounted) context.go(sb != null ? '${AppRoutes.dashboard}?companyId=$sb' : AppRoutes.dashboard);
      }
    } catch (e) {
  ...
  Future<String?> _getCompanyId() async {
    try {
      final sb = await Future.value(null); // handled in splash
      return null;
    } catch (_) { return null; }
  }
          ...
          child: Column(
            children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                TextButton(onPressed: () => context.go(AppRoutes.register), child: const Text('Sign up')),
              ]),
            ],
          ),
        ),
...
class _RegisterScreenState extends ConsumerState<RegisterScreen> {
  ...
  Future<void> _submit() async {
    ...
    setState(() { _loading = true; _error = null; });
    try {
      final res = await AuthService.instance.signUp(email: _email.text.trim(), password: _pass.text);
      if (!mounted) return;
      if (res.session != null) {
        context.go(AppRoutes.onboarding);
      } else {
        setState(() { _loading = false; _error = 'Check your email and confirm your account, then sign in.'; });
      }
    } catch (e) {
      setState(() { _loading = false; _error = 'Registration failed: ${e.toString()}'; });
    }
  }
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 6),
              Text('Get started in minutes', style: Theme.of(context).textTheme.bodyMedium),
              const SizedBox(height: 32),
              _AuthField(ctrl: _email, label: 'Email', hint: 'you@company.com', keyboard: TextInputType.emailAddress),
