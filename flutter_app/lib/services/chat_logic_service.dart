...
class ChatLogicService {
    ...
    try {
      ...
      final matched = intents.firstWhere(
        ...
        orElse: () => Intent(
          ...
          response: data['response'] ?? '', createdAt: DateTime.now(), updatedAt: DateTime.now(),
        ),
      );
      return IntentMatchResult(
        intent: matched, confidence: (data['confidence'] as num).toDouble(),
  ...
  // ── Stage 4: GPT-4o-mini ─────────────────────────────────
  Future<String?> askAI({
    required String message, required Company? company,
    ...
    try {
      await http.post(
        ...
        body: jsonEncode({
          ...
          'intents': intents.where((i) => i.isActive && i.trainingPhrases.isNotEmpty).map((i) => {
            'id': i.id, 'name': i.name,
